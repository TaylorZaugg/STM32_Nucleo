...
void AddressOperation(char address);
char ReadOperation(char address);
...
int main(){
	...
	InitI2C();
	//Define expander addressses
	char IODIR = 0x00;
	char GPIO = 0x09;
	char INTF = 0x07; 	 //INTF – INTERRUPT FLAG REGISTER (ADDR 0x07)
	char GPINTEN = 0x02; //GPINTEN – INTERRUPT-ON-CHANGE PINS (ADDR 0x02)
	char IPOL = 0x01; //IPOL – INPUT POLARITY PORT REGISTER (ADDR 0x01)
	char DEFVAL= 0x3; // 0x03			DEFVAL	- Default Value Register
	char IOCON = 0x05; //0x05			IOCON	- I/O Expander Configuration Register
	char INTCON = 0x04; //0x04			INTCON	- Interrupt-onChange Control Register
	...
	// Expander config
	WriteOperation(IODIR, IODIRconfig);// Configure LED output and Switches as input
	WriteOperation(IPOL, 0x0F); //Flip polarity of switches
	WriteOperation(GPINTEN, 0xF0); // Set interrupt for first 4 switches on change
	WriteOperation(IOCON, 0x2); //INT output high 1 = Active-high, bit 1
	//=======================================================================

	//testing
	char currentLEDs = 0;
	WriteOperation(GPIO, currentLEDs); //default to off before setting based on switches

	//Test read operation
	char gpiodata = ReadOperation(GPIO);
	//test, mostly debug for reading output value
	currentLEDs = gpiodata<<4; //Is shifting right way?
	WriteOperation(GPIO, currentLEDs);

	while(1){
		//Polling for testing/ brute force, ideally use interrupt later
		char gpiovals = ReadOperation(GPIO);
		gpiovals = gpiovals << 4; //shift switch values to GPIO position, needs testing e.g. 11110000 >> 00001111
		WriteOperation(GPIO,gpiovals);
	}
}
...
char ReadOperation(char address){
	...
	//Write to the address without data, prep for where want to read from
	...
	I2C1->CR2 |= 1 << 16; // Want to read 1 byte. So NBYTES = 1

	// 6- Issue the START condition
	bitset(I2C1->CR2, 13); // START bit ON

	// 9- Wait until RXNE
	while(bitcheck(I2C1->ISR, 2) == 0 );	//GETTING STUCK HERE, maybe never get response
